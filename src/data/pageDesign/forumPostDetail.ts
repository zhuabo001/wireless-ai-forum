import type { PostDetail, ResourceLink, CommentSortOption, Comment } from '@/types/pageDesign/forumPostDetail'
...
// 贴文正文以 markdown 原文维护（模拟编辑器提交内容），
// postContentHtml 模拟内部项目后端返回的 page.content（整段 HTML 字符串）
export const postContentMarkdown: string = `在5G NR的MAC层调度中，传统的比例公平（PF）算法虽然能够保证基本的吞吐量与公平性平衡，但在面对高动态负载、多样化业务类型（eMBB/URLLC/mMTC）共存时，其固定的启发式规则往往难以达到最优。本文分享我们团队在过去一年中，将深度强化学习（DRL）引入5G调度决策的完整实践路径，包括仿真环境搭建、算法选型、训练策略，以及最终在某城市热点区域的现网试点结果。

## 一、问题背景与动机

5G网络需要同时支撑增强移动宽带（eMBB）、超可靠低时延通信（URLLC）和海量机器类通信（mMTC）三大类业务。不同业务对吞吐量、时延、可靠性的需求差异巨大：

- eMBB 业务追求峰值速率，对时延不敏感
- URLLC 业务要求端到端时延低于 1ms，可靠性 99.999%
- mMTC 业务连接密度高，但单次传输数据量极小

传统 PF 算法以 \`R_i(t) / ̅R_i(t)\` 作为优先级度量，无法根据业务类型动态调整调度策略。我们在现网观测到，在 URLLC 与 eMBB 并发场景下，PF 算法的 URLLC 丢包率比理论最优高出约 18%。

## 二、强化学习方案设计

### 2.1 状态空间（State Space）

我们将每个 TTI（Transmission Time Interval）的调度决策建模为一个 MDP 过程。状态向量包含以下维度：
...
动作定义为每个资源块组（RBG）分配给哪个 UE，以及对应的 MCS 等级选择。为降低动作空间维度，我们采用分层的动作结构：先选择 UE，再在该 UE 支持的 MCS 集合中选择最优等级。

### 2.3 奖励函数（Reward）

奖励设计是本项目最关键的部分。我们采用多目标加权的方式：
...
其中权重 \`w1-w4\` 通过网格搜索在验证集上调优。值得注意的是，我们没有使用固定的权重，而是训练了一个辅助的元网络（Meta-Network）根据网络负载动态调整权重，这使得算法在轻载和重载场景下都能保持较好的适应性。

## 三、仿真环境与训练

仿真环境基于 3GPP TR 38.901 信道模型搭建，关键配置如下：

| 参数 | 配置 |
|---|---|
| 载波频率 | 3.5 GHz (n78) |
| 系统带宽 | 100 MHz (273 RBs) |
| 子载波间隔 | 30 kHz |
| UE数量 | 50-200（动态接入） |
| 业务模型 | Full-buffer + Poisson arrival |
| 移动性 | 3 km/h pedestrian, 30 km/h vehicular |

我们对比了 PPO、SAC 和 A3C 三种算法。在相同的训练步数（10M steps）下，SAC 在收敛稳定性和最终性能上均优于其他两种算法。推测原因与 SAC 的最大熵框架有关——它鼓励策略探索，避免过早陷入局部最优。

> "强化学习在无线通信中的应用，最大的挑战不是算法本身，而是如何设计一个既贴近现网、又足够高效的仿真环境。过度简化的信道模型会导致 sim-to-real 差距过大。"

## 四、现网试点结果

在某一线城市的商业热点区域，我们将训练好的策略网络部署到 gNB 的 MAC 层调度模块中（通过 ONNX 格式导出，推理时延约 0.3ms，满足 TTI 级调度要求）。

与基线 PF 算法相比，关键指标提升如下：

- **小区平均吞吐量**：+12.3%
- **URLLC 丢包率**：-23.7%（从 0.095% 降至 0.072%）
- **边缘用户吞吐量（5th percentile）**：+18.6%
- **Jain 公平性指数**：从 0.82 提升至 0.89
...
1. **Sim-to-real gap**：仿真中的 CQI 上报是完美的，现网存在测量误差和上报延迟。我们在训练阶段加入了高斯噪声和随机延迟来增强鲁棒性。
2. **训练稳定性**：早期训练经常出现策略崩溃（policy collapse），所有资源都分配给某一个 UE。通过加入熵正则和 reward shaping 缓解。
3. **计算资源**：在 200 UE 场景下，单次推理需要评估的动作空间过大。我们采用 action masking 技术，只评估缓存非空且信道条件允许的 UE-MCS 组合，将计算量降低了约 70%。
4. **在线更新**：无线环境随时间变化（用户行为、干扰源），离线训练的策略会退化。我们正在探索轻量级的在线微调方案，每周用现网收集的数据进行几轮 PPO 更新。

完整的仿真代码和训练脚本已开源在 GitHub（链接见文末）。欢迎社区同学一起探讨，尤其是在多智能体协作调度方向的思路。
`

export const postContentHtml: string = renderMarkdown(postContentMarkdown)
