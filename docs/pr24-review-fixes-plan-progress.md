...
| 8. vditor 运行时资源改为 vite-plugin-static-copy 构建期拷贝，删除 public/vditor 入库资源 | 完成 | refactor: copy vditor runtime assets from node_modules at build time — eb288ae（dev 中间件与 build 产物均实测 200；干净 clone npm ci + build 通过；分支事故说明：该提交曾误挂 codex 分支，已通过 feat 快进 + codex rebase --onto 迁移修正） |
