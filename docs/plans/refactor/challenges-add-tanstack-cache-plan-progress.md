...
| 复核修复 | 完成 | 独立复核（general-purpose agent 全量审查 9f04840..HEAD）：无 Critical；2 Important + 7 Minor 全部处理 | 修复后 `npm run check` 全绿（56 例）；dev 实证：page 2 切 tab 仅 1 次请求（flush:'sync' 归页先于 key 迁移）、切角色与揭榜全程 0「加载中」折叠帧（keepPreviousData） | Important-1 列表页 page>1 改过滤双请求（中间 key）→ 归页 watch 加 `{ flush: 'sync' }`；Important-2 详情 key 迁移整页折叠 + 浪费 GET → `placeholderData: keepPreviousData`；Minor-2 404 文案透出 error.message + comments 查询在 detail 404 时禁用；Minor-3 修复空断言；Minor-4 补 queryClient retry 单测 2 例；Minor-5 抽 invalidateChallengeAfterMutation；Minor-1（meta 失败静默）已知限制：mock 下 meta 恒成功，失败态留待真实后端联调时补 |
